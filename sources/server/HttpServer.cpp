...
void HttpServer::init() {
  ...
  this->_hServer.resource["^/client_list$"]["POST"] = [this](std::shared_ptr<HttpsServer::Response> response,
                                                             std::shared_ptr<HttpsServer::Request> request) {
      this->client_list(response, request);
  };
  this->_hServer.resource["^/send$"]["POST"] = [this](std::shared_ptr<HttpsServer::Response> response,
                                                      std::shared_ptr<HttpsServer::Request> request) {
...
void HttpServer::client_list(std::shared_ptr<HttpsServer::Response> response,
                             std::shared_ptr<HttpsServer::Request> request){
  try {
    pt::ptree iptree;
    pt::read_json(request->content, iptree);
...
void HttpServer::send(std::shared_ptr<HttpsServer::Response> response,
                      std::shared_ptr<HttpsServer::Request> request){
  try {
    pt::ptree iptree;
    pt::read_json(request->content, iptree);
    ...
    if (this->isConnected(iptree.get<std::string>("cookie"))) {
      this->_server->send(json::stringify(iptree.get_child("data")),
                          iptree.get<std::string>("client_cookie"));
      optree.put("status", "ok");
    } else {
      optree.put("status", "ko");
    }
